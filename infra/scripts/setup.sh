...
# State registry — the STATES table (Display Label|slug|fips|usps|viewbox) and the
# state_resolve helper, shared with the deploy-scope env derivation so the picker
# here and a single-state production deploy use the same bboxes.
# shellcheck source=/dev/null
. "${SCRIPT_DIR}/state-registry.sh"
...
#
# The STATES table (Display Label|geofabrik-slug|state-fips|usps-code|viewbox) is
# defined in state-registry.sh, sourced above. viewbox is the state's bounding box
# in Nominatim order — min_lng,max_lat,max_lng,min_lat (left,top,right,bottom) —
# used as GEOCODER_VIEWBOX (bounded geocoder search) AND the initial map framing
# for a single-state build.
